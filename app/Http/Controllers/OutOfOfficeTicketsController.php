...
class OutOfOfficeTicketsController extends Controller
{
    ...
    public function Show_Pending_Tickets(){ 
        if (auth()->user()->hasRole('ROLE_SUPER_ADMIN') ) {
            ...
            $tickets_waiting_approval = Out_Of_Office_Tickets_Model::whereIn('status', ['2'])
            ...
            $all_tickets = Out_Of_Office_Tickets_Model::with('user_owner')
            ...
            $tickets_waiting_approval = Out_Of_Office_Tickets_Model::where([
                ['status', ['2']],
                ['user_id', auth()->id()],
                ])->get();
