<div class="d-flex justify-content-end">
    {{ $pending_tickets->withQueryString()->links('pagination::bootstrap-5') }}
</div>

<table id="pending-bbnt-tickets-table" class="common-table" width="100%" >
    <thead>
        <th width="5%"></th>
        <th width="15%">Title</th>
        <th width="15%">User Owner</th>
        <th width="15%">Status</th>
        <th width="15%">Onsite Type</th>
        

    </thead>

    <tbody>
        @foreach ($pending_tickets as $ticket)
            <tr>
                <td>
                    
                    <a href="/bbnt-ticket-details/{{ $ticket->id }}">
                        <button><i class="ti-arrow-right" ></i></button>
                    </a>
                    
                </td>
                <td>{{ $ticket->document_name }}</td>
                <td>{{ $ticket->user_owner->fullname }}</td>
                <td>
                    <span class="badge rounded-pill bg-{{ $ticket->status_data['color'] ?? 'primary' }} px-3 py-2">
                        {{ $ticket->status_data['text'] }}
                    </span>
                </td>
                <td>
                    <span class="badge rounded-pill bg-{{ $ticket->onsite_type_data['color'] ?? 'primary' }} px-3 py-2">
                        {{ $ticket->onsite_type_data['text'] }}
                    </span>
                </td>

            </tr>
        @endforeach
    </tbody>
</table>